...
namespace NetCams
{
    public class MultiplicationOperationInstance : BasicMathOperationInstance
    {
        public MultiplicationOperationInstance(MultiplicationOperationDefinition theDefinition, TestExecution testExecution)
        ...
        public override void DoWork()
        {
            ...
            switch (Result.Type)
            {
                case DataType.DecimalNumber:
                    double doubleValue = ValueObjects[0].ValueAsDecimal(); // TODO: is there an accuracy issue here?  We are multiplying the requested values by 1.000000... ideally we would init to one of the values
                    for (int x = 0; x < OperatorsUsed.Count; x++)
                    {
                        switch (OperatorsUsed[x])
                        {
                            ...
                            case "/":
                                ...
                                break;
                            default:
                                throw new ArgumentException("Invalid operator in MultiplicationOpInstance; op='" + OperatorsUsed[x] + "'");
                                break;
                        }
                    }
                ...
                default:
                    throw new ArgumentException("Data type " + Result.Type + " not supported by multiplication operation.");
            }
